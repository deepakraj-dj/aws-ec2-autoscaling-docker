...
├── terraform/
│   ├── main.tf                    # Creates EC2 launch template with IAM & security settings
│   ├── outputs.tf                 # Displays launch template ID after deployment
│   ├── user_data.sh               # Auto-installs Docker, AWS CLI, pulls Nginx image from ECR
│   ├── vars.tf                    # Defines all variables (region, instance type, etc.)
│   └── terraform.tfvars           # Actual values for variables (add to .gitignore)
